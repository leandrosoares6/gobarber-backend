...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    /*
     * Check if user defines provider_id with his own id
     */

    if (provider_id === req.userId) {
      return res.status(400).json({
        error: 'User cannot defines provider_id with his own id',
      });
    }
    ...
    const matching = date.split('T');
    const match = matching[1].split(':');
    console.log(match[0]); */
  ...
  async delete(req, res) {
    ...
    const validDate = subHours(appointment.date, 2);

    if (!isBefore(new Date(), validDate)) {
      return res.status(400).json({
